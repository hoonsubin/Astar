...
fn new_era_is_ok() {
    ExternalityBuilder::build().execute_with(|| {
        // prepare stash-controller pair with some bonded funds
        let mut block_number = BlockPerEra::get() - 2;
        const CURRENT_ERA: crate::EraIndex = 3;

        // set initial era index
        <CurrentEra<TestRuntime>>::put(CURRENT_ERA);

        // increment era index, but make sure it is still not last block in the era
        // call on_initilize. this time it should be last block in the erra and Current should be incremented
